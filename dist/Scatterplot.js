...
class Scatterplot extends React.Component {
  constructor(props) {
    ...
    let x, y, hue, size; // let size = 'popularity'
    ...
    y.type = [typeof y.accessor(this.props.data[0])].includes(['int', 'float']) ? 'numeric' : 'categorical'; // hue.type = [typeof(hue.accessor(this.props.data[0]))].includes(['int', 'float']) ? 'numeric' : 'categorical'
  ...
  updateSize() {
    ...
    let yMax = this.props.yMax ?? d3.max(this.props.data, d => +this.state.y.accessor(d)); // console.log(this.svgRef)
  ...
  updateScales() {
    ...
    console.log('scale state:', this.state); // this.xMin = this.props.xMin ?? d3.min(this.props.data, (d) => +x.accessor(d));
    ...
    this.sizeMax = d3.max(this.props.data, d => size.accessor(d)); // if (this.state.x.scale.domain() != [this.state.xMin, this.state.xMax])
    // Define scales
    // if (this.state.drawWidth != 0) {

    x.scale = x.scale.domain([this.state.xMin, this.state.xMax]).range([this.props.margin.left, this.state.drawWidth]); // Nice?

    if (this.props.nice == 'nice') x.scale = x.scale.nice();
    if (x.scale != this.state.x.scale) this.setState({
      x: x
    }); // }

    y.scale = y.scale.domain([this.state.yMin, this.state.yMax]).range([this.state.drawHeight, this.props.margin.top]); // Nice?

    if (this.props.nice == 'nice') y.scale = y.scale.nice();
    if (y.scale != this.state.y.scale) this.setState({
      y: y
    });
    hue.scale = hue.scale.domain([this.hueMin, this.hueMax]);
    if (hue.scale != this.state.hue.scale) this.setState({
      hue: hue
    });
    size.scale = size.scale.domain([this.sizeMin, this.sizeMax]).range([this.props.radius * 1.25, this.props.radius * 0.75]);
    if (size.scale != this.state.size.scale) this.setState({
      scale: scale
    });
  }
  ...
  updatePoints() {
    ...
    let circles = d3.select('.chart-group').selectAll('circle').data(this.props.data, d => d.id); // Use the .enter() method to get your entering elements, and assign their positions

    let circlesEnter = circles.enter().append('circle').attr('class', 'chart-item').on('mouseover', this.props.setTooltip ? (e, d) => this.props.setTooltip(d) : null).on('mouseout', this.props.setTooltip ? (e, d) => this.props.setTooltip(undefined) : null).attr('cx', d => {
    ...
    }).attr('cy', d => this.state.y.scale(this.state.y.accessor(d))).attr('r', d => this.state.size ? this.state.size.scale(this.state.size.accessor(d)) : this.props.radius).style('fill-opacity', 0); // circles.transition().duration(this.props.transitionSpeed)
    ...
    circlesEnter.merge(circles).on('click', d => this.props.onClickChartItem(d)) // .attr('cx', d => this.state.x.scale(this.state.x.accessor(d)) )
    // .attr('cy', d => this.state.y.scale(this.state.y.accessor(d)) )
    .transition().duration(this.props.transitionSpeed).attr('fill', d => this.state.hue ? this.state.hue.scale(this.state.hue.accessor(d)) : this.props.color).attr('r', d => this.state.size ? this.state.size.scale(this.state.size.accessor(d)) : this.props.radius).attr('cx', d => this.state.x.scale(this.state.x.accessor(d))).attr('cy', d => this.state.y.scale(this.state.y.accessor(d))).style('fill-opacity', d => this.state.drawWidth > 0 ? 1 : 0).style('stroke', 'black').style('stroke-width', 0.5).style('stroke-opacity', d => this.state.drawWidth > 0 ? 1 : 0); // Use the .exit() and .remove() methods to remove elements that are no longer in the data
  ...
  updateAxes() {
    ...
    let xAxisFunction = d3.axisBottom().scale(this.state.x.scale).ticks(this.props.xTicks).tickFormat(this.props.xFormat);
    let yAxisFunction = d3.axisLeft().scale(this.state.y.scale).ticks(this.props.yTicks).tickFormat(this.props.yFormat);
    d3.select('.x-axis-g').transition().duration(this.props.transitionSpeed).call(xAxisFunction);
    d3.select('.y-axis-g').transition().duration(this.props.transitionSpeed).call(yAxisFunction);
  }
  ...
  update() {
    ...
    this.updateSize();
    this.updateScales();
    if (this.state.drawWidth > 0) this.updateAxes();
    if (this.state.drawWidth > 0) this.updatePoints();
  }
  ...
  render() {
    ...
    return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("svg", {
      ...
      transform: `translate(${this.state.drawWidth / 2},0)`
    }, this.props.title), /*#__PURE__*/React.createElement("g", {
      className: "x-axis-g",
      transform: `translate(${0}, ${this.state.drawHeight})`
    }), /*#__PURE__*/React.createElement("g", {
      className: "y-axis-g",
