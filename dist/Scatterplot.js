...
export default function Scatterplot({
  ...
  useEffect(() => {
    ...
    background.enter().append('rect').attr('class', 'chart-container').attr('x', 0).attr('y', 0).attr('width', '100%').attr('height', '100%').attr('opacity', 0);
    // .on('click', d => onClickBackground(d))

    // Select all circles and bind data
    let circles = d3.select('.chart-container').selectAll('circle').data(data, keyBy ? keyBy : null);

    // Use the .enter() method to get your entering elements, and assign their positions
    let circlesEnter = circles.enter().append('circle').attr('class', 'chart-item').style('cursor', 'pointer').attr('cx', d => xAxis.scale(xAxis.accessor(d))).attr('cy', d => yAxis.scale(yAxis.accessor(d)))
