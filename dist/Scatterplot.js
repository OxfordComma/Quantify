...
class Scatterplot extends React.Component {
  ...
  updateObjects() {
    ...
    if (!x.min) x.min = d3.min(this.props.data, d => +x.accessor(d));
    if (!x.max) x.max = d3.max(this.props.data, d => +x.accessor(d)); // Strings are in case the min is a date
    ...
    if (!y.min) y.min = d3.min(this.props.data, d => +y.accessor(d));
    if (!y.max) y.max = d3.max(this.props.data, d => +y.accessor(d));
    console.log({
      y: y
    });
    ...
    if (this.props.hue.scale) {
      ...
      if (scaleType == 'number' || hue.accessor(this.props.data[0]) instanceof Date) hue.scale = d3.scaleLinear();else hue.scale = d3.scaleOrdinal();
    }
    ...
    if (scaleType == 'number' || size.accessor(this.props.data[0]) instanceof Date) size.scale = d3.scaleLinear();else size.scale = d3.scaleBand(); // }
  ...
  updateScales() {
    ...
    // X Domain
    let x = this.state.x;
    if (!x?.scale) return; // Check if Date
    ...
    y.scale.range([this.state.drawHeight, this.props.marginTop]); // this.setState({y: y})
    // }
    // Update Hue
    ...
    if (!size?.scale) return; // Update Size
    ...
    if (scaledSizeType == 'number') {
      ...
      if (size.scale.domain().length > 1) size?.scale.range([this.props.radius * 1.25, this.props.radius * 0.75]);else size?.scale.range([this.props.radius, this.props.radius]);
    }
  }
  ...
  render() {
    ...
    if (!this.state.x === undefined || !this.state.y === undefined) return /*#__PURE__*/React.createElement("div", null);
    return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("svg", {
      ...
      className: "y-axis-g",
      transform: `translate(${this.props.marginLeft}, ${0})`
    }))));
  }
