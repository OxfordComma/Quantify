...
export default function Scatterplot({
  ...
  function detectScaleType(datum, accessor) {
    if (accessor(datum) instanceof Date) return 'date';else return typeof accessor(datum);
  }

  function instantiateX() {
    if (typeof x == 'string') {
      x = {
        name: x // accessor: d => d[x.name],
    ...
    if (x.scale) {
      ...
      if (scaleType == 'number' || scaleType == 'date') {
        domain = [x.min, x.max]; // console.log('x linear', domain)

        x.scale = d3.scaleLinear();
        x.span = x.max - x.min;
      } else {
        domain = [...new Set(data.map(d => x.accessor(d)))].sort();
        x.scale = d3.scalePoint(); // console.log('x ordinal', domain)
      ...
      x.scale.range([marginLeft, dimensions.drawWidth - marginRight]); // x.scale.nice()

      if (!x.format) {
        if (scaleType == 'date') {
          x.format = dateFormat;
        } else if (scaleType == 'number') x.format = format;
      }
    }
  ...
  function instantiateHue() {
    ...
    if (hue.scale) {
      ...
      let scaleType = detectScaleType(data[0], hue.accessor); // console.log('hue scale type:', scaleType)
  ...
  function instantiateSize() {
    ...
    if (size.scale) {
      ...
      if (scaleType == 'number' || scaleType == 'date') {
        ...
        if (!size.min) size.min = d3.min(data, d => +size.accessor(d));
        if (!size.max) size.max = d3.max(data, d => +size.accessor(d));
        ...
        if (domain.length > 1) size.scale.range([radius * 1.25, radius * 0.75]);else size.scale.range([radius, radius]);
      }
    }
  ...
  useEffect(() => {
    let background = d3.select('.chart-container').selectAll('rect').data([null]); // Create the transparent background width

    background.enter().append('rect').attr('class', 'chart-container').attr('x', 0).attr('y', 0).attr('width', '100%').attr('height', '100%').attr('opacity', 0); // .on('click', d => onClickBackground(d))
    // Select all circles and bind data

    let circles = d3.select('.chart-container').selectAll('circle').data(data, keyBy ? keyBy : null); // Use the .enter() method to get your entering elements, and assign their positions

    let circlesEnter = circles.enter().append('circle').attr('class', 'chart-item').style('cursor', 'pointer').attr('cx', d => xAxis.scale(xAxis.accessor(d))).attr('cy', d => yAxis.scale(yAxis.accessor(d))) // .on('mouseover', tip.show)
    // .on('mouseout', tip.hide)
    // .on('mouseover', setTooltip ? (e, d) => setTooltip(d) : null)
    // .on('mouseout', setTooltip ? (e, d) => setTooltip(undefined) : null)
    .style('fill-opacity', 0); // console.log('hue', xAxis.accessor(1))

    circlesEnter.merge(circles).on('click', d => onClickChartItem(d)).transition().duration(transitionSpeed).attr('fill', d => fill ? d[fill] : hueAxis ? hueAxis.scale(hueAxis.accessor(d)) : color).attr('r', d => sizeAxis ? sizeAxis.scale(sizeAxis.accessor(d)) : radius).attr('cx', d => xAxis.scale(xAxis.accessor(d))).attr('cy', d => yAxis.scale(yAxis.accessor(d))).style('fill-opacity', d => dimensions.width > 0 ? 1 : 0).style('stroke', 'black').style('stroke-width', 0.5).style('stroke-opacity', d => dimensions.width > 0 ? 1 : 0); // Use the .exit() and .remove() methods to remove elements that are no longer in the data

    circles.exit().transition().duration(transitionSpeed).style('fill-opacity', 0).remove();
  }, [data, xAxis, yAxis, hueAxis, sizeAxis]);
  useEffect(() => {
    // let scaleType = detectScaleType(data[0], )
    let xAxisTicks = xTicks; // let xDomainSpan = xAxis.scale.domain()[1] - xAxis.scale.domain()[0]

    if (xAxis.span < 10) {
      xAxisTicks = xAxis.span;
    }

    let yAxisTicks = yTicks; // let yDomainSpan = yAxis.scale.domain()[1] - yAxis.scale.domain()[0]
    ...
    let xAxisFunction = d3.axisBottom().scale(xAxis.scale).ticks(xAxisTicks).tickFormat(xAxis.format);
    let yAxisFunction = d3.axisLeft().scale(yAxis.scale).ticks(yAxisTicks).tickFormat(yAxis.format);
